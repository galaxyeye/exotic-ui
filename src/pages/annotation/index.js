...
import {DataGrid, GRID_CHECKBOX_SELECTION_FIELD, GridToolbar,} from '@mui/x-data-grid';
...
import {useHarvestData} from "./useHarvestData";
import {Container, Divider, Popper, Typography} from "@mui/material";
...
SettingsPanel.propTypes = {
    ...
    type: PropTypes.oneOf(['Harvest']).isRequired,
};

export default function AnnotationDefault() {
    const [anchorEl, setAnchorEl] = React.useState(null);
    const [currentCellValue, setCurrentCellValue] = React.useState('');
    const [columnLabel, setColumnLabel] = React.useState('');

    const [isAntDesign, setIsAntDesign] = React.useState(false);
    const [type, setType] = React.useState('Harvest');
    const [size, setSize] = React.useState(100);
    const [modifiedDataJson, setModifiedDataJson] = React.useState([]);

    const tableId = 0
    const {loading, data, setRowLength, loadNewData} = useHarvestData({
        dataSet: type,
        tableId: tableId,
        rowLength: size,
        maxColumns: 40,
    ...
    const getLatestData = (rowId, field) => {
        const row = latestData.rows.find((r) => r.id == rowId)
        return row[field]
    }

    const setLatestData = (rowId, field, value) => {
        const labelRow = latestData.rows.find((r) => r.id == -1)
        const label = labelRow[field]
        modifiedData.push({tableId: tableId, label: label, rowId: rowId, field: field, value: value})
        setModifiedDataJson(JSON.stringify(modifiedData, null, 4))
    ...
    const handlePopoverOpen = (event) => {
        ...
        setCurrentCellValue(event.currentTarget.textContent)
    ...
    const findDataRowByField = (event) => {
        const field = event.currentTarget.dataset.field;
    ...
    return (
        <Box>
            <StyledBox>
                ...
                <DataGridComponent
                    ...
                    onCellEditCommit={(params, event) => {
                        setLatestData(params.id, params.field, params.value)
                        if (params.id === -1) {
                            setColumnLabel(params.value)
                        } else {
                            // event.target.textContent = params.value
                            setCurrentCellValue(params.value)
                        }
                    }}
                />
            </StyledBox>
            ...
            <Box>
                <Typography sx={{p: 1}}>{columnLabel}</Typography>
                <Typography sx={{p: 1}}>{currentCellValue}</Typography>
                <>
                ...
                >
                    <Typography sx={{ border: 1, p: 1, bgcolor: 'background.paper' }}>{currentCellValue}</Typography>
                </Popper>
            </Box>
        </Box>
    );
